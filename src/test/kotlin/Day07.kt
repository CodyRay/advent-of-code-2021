...
fun nextGeneration(currentState: List<Int>) =
    currentState.map { it - 1 }.flatMap { if (it < 0) listOf(6, 8) else listOf(it) }

fun nGenerations(generations: Int, startState: List<Int>): List<List<Int>> {
    return (1..generations)
        .runningFold(startState) { state, _ -> nextGeneration(state) }
}
...
class Day07 {
    @Test
    fun main() {
        val input = parsed(readPuzzleInputLines("Day07").single())
